...
scout_Collection.prototype = {
	...
	,add: function(model) {
		...
		if(!Lambda.has(this.models,model)) {
			...
			var this1 = this.modelListeners;
			var value = model.observe(function(_) {
				scout__$Signal_Signal_$Impl_$.dispatch(_gthis.onChange,model);
				return;
			});
			this1.set(model,value);
...
scout_PropertyOfObservable.__name__ = true;
...
var todo_model_Todo = function(props) {
	...
	this.props.editing = new scout_Property(props.editing != null && props.editing);
	var tmp;
	if(props.id != null) {
		tmp = props.id;
	} else {
		var tmp1 = todo_model_Todo;
		tmp1.__scout_ids += 1;
		tmp = tmp1.__scout_ids;
	}
	this.props.id = new scout_Property(tmp);
