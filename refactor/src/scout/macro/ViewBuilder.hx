...
class ViewBuilder {
      ...
      case FFun(func):
        ...
        if (f.meta.hasEntry(observeMetaNames)) {
          ...
          for (meta in metas) {
            ...
            initializers.push(Common.makeObserverForState('states', meta.params[0], macro this.$name));
          }
        }
        true;
      default: true;
  ...
  function makeFieldsForState(f:Field, t:ComplexType, ?e:Expr):Array<Field> {
    var isOptional = f.meta.hasEntry([ ':optional' ]) || e != null;
    states.push(Common.makeState(f.name, t, f.pos));
    constructorFields.push(Common.makeConstructorField(f.name, t, f.pos, isOptional));
    attrInitializers.push(Common.makeStateInitializer('attrs', 'states', f.name, t, e));
    return [
      Common.makeProp(f.name, t, f.pos, true),
      Common.makeStateGetter('states', f.name, t, f.pos),
      Common.makeStateSetter('states', f.name, t, f.pos)
    ];
  }
